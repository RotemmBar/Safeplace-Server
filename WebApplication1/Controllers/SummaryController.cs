    ...
    public class SummaryController : ApiController
    {
        ...
        public IHttpActionResult GetSummaryByNum(string PatientId, string date)
        {
            try
            {
                ...
                SummaryDto Summary = db.TblSummary.Where(a => a.Summary_Date.ToString().Substring(0, 10) == date && a.WrittenBy == "t" && a.TblTreatment.FirstOrDefault().TblTreats.FirstOrDefault().Patient_Id == PatientId).Select(x => new SummaryDto()
                {
